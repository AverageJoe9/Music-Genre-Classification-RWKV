...
class MusicRWKV(nn.Module):
    ...
    def forward(self, x, targets=None):
        ...
        #
        #     if '32' in os.environ['RWKV_FLOAT_MODE']:
        #         c = c @ F.one_hot(x, num_classes=self.config.vocab_size)
        #     elif os.environ['RWKV_FLOAT_MODE'] == 'fp16':
        #         c = c @ F.one_hot(x, num_classes=self.config.vocab_size).half()
        #     elif os.environ['RWKV_FLOAT_MODE'] == 'bf16':
        #         c = c @ F.one_hot(x, num_classes=self.config.vocab_size).bfloat16()
        #
        #     x = self.head(x) + c
        # else:
        #     x = self.head(x)
